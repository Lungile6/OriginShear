...
export default function ValidatorPending() {
  ...
  return (
    <div className="min-h-dvh bg-background">
      <TopAppBar role="AUTH" />
      <div className="bg-role-validator text-white px-margin-mobile pt-20 pb-12 text-center">
        ...
        <p className="text-body-sm text-white/80">
          Your wallet is connected but your{" "}
          <span className="font-mono font-bold text-primary-fixed">VALIDATOR_ROLE</span> has not been
          assigned yet.
        </p>
      </div>
      ...
      <div className="px-margin-mobile -mt-6">
        <Card>
          ...
          <a
            href={`mailto:${SUPPORT.email}?subject=${encodeURIComponent("ORIGINSHEAR Validator Support")}`}
            className="block"
          >
        ...
        <div className="space-y-stack-md mt-stack-md mb-8">
          <InfoCard border="border-l-role-farmer" title="Policy" desc="Validators must adhere to the Lesotho Agriculture Digital Standards (LADS) for all data certification tasks." />
          <InfoCard border="border-l-role-validator" title="Compliance" desc="The Ministry of Agriculture requires active biometric identity verification for all institutional roles." />
          <InfoCard border="border-l-error" title="Security" desc="Never share your private keys. Only the public wallet address shown above is required for role assignment." />
        </div>
      </div>
    </div>
  );
